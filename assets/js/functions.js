...
function rate_and_remove1(e) {
  ...
  xhttp2.open('POST', "http://127.0.0.1:5000/watchlist/delete/1", true);
  xhttp2.setRequestHeader('Content-type', "application/json;charset=UTF-8");
  let send2 = {id: "1", title: temp_database_after_watched[temp_length][0]};
...
function getApi() {
  ...
  $.ajax({
    method: "GET",
    url: "https://imdb-api.com/en/API/SearchAll/k_ysik8czp/" + getTitle,
...
function get_recommended(){
  ...
  for(let i=0; i<temp_database_after_watched.length; i++){
    let html4 = `<button style="width: 100%;">${temp_database_after_watched[i][0]}</button>`
    document.querySelector(".portfolio").querySelector(".container").querySelector(".dropdown").querySelector('.dropdown-content').insertAdjacentHTML("beforeend", html4)
  }  
}